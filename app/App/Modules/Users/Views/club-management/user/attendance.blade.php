                ...
                <div class="col-lg-8 col-md-11">
                    @foreach($groupData['data'] as $key => $dataItem)
                        @if($dataItem->year == $year)
                            ...
                            {{ Form::hidden('data[' . $key . '][year]', $dataItem->year) }}
                            {{ Form::hidden('data[' . $key . '][month]', $dataItem->month) }}
                                    ...
                                    @foreach($dataItem->group->trainingDays($dataItem->year, $dataItem->month) as $day)
                                        <td class="text-center">
                                            <label for="attendance_{{ $dataItem->user_id }}_{{ $dataItem->group_id }}_{{ $dataItem->month }}_{{ $day->day }}" style="width: 100%; height: 100%;">
                                                {{ Form::hidden('data[' . $key . '][data][' . $dataItem->group_id . '][attendance][' . $day->day . ']', 0) }}
                                                {{ Form::checkbox('data[' . $key . '][data][' . $dataItem->group_id . '][attendance][' . $day->day . ']', 1, $dataItem->attendance($day->day) == '1' ? true : false, array('id' => 'attendance_' . $dataItem->user_id . '_' . $dataItem->group_id . '_' . $dataItem->month . '_' . $day->day)) }}
                                            </label>
                                        </td>
                                    @endforeach
                                </tr>
